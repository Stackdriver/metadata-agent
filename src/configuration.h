...
class Configuration {
 public:
  ...
  NOOPT const std::string& MetadataApiBindAddress() const {
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  NOOPT int KubernetesUpdaterWatchConnectionRetries() const {
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  NOOPT int HealthCheckMaxDataAgeSeconds() const {
    std::lock_guard<std::mutex> lock(mutex_);
