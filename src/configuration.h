...
class Configuration {
 public:
  ...
  int KubernetesUpdaterWatchReconnectMaxSilenceSeconds() const {
    std::lock_guard<std::mutex> lock(mutex_);
    return kubernetes_updater_watch_reconnect_max_silence_seconds_;
  }
 ...
 private:
  ...
  int kubernetes_updater_watch_reconnect_max_silence_seconds_;
